...
Cada partida SHALL clasificarse en exactamente una categoría: la del tema practicado cuando la sesión usó uno solo —`fracciones`, `decimales` o `algebra`— y `mixto` cuando combinó más de uno. La categoría SHALL derivarse de la selección, sin que el jugador la elija aparte. El conjunto de categorías SHALL mantenerse alineado entre el cliente y el servidor: un tema que exista en el registro y no tenga categoría propia en el servidor haría que sus puntajes se archivaran bajo otro contenido.
