...
- `src/lib/topics/` is the single source of truth for which topics exist. Each topic is a self-contained folder (`fracciones/`, `decimales/`, `algebra/`, `problemas/`) exporting generators, a `Render` component, and its `describe`/`hint` text, registered in `topics/index.ts`. **Adding a topic is a new folder plus one line in that index** — no changes to the game screens.
...
- **`problemas` is the transversal topic**: word problems whose statement never says which technique solves them, because deciding that *is* the exercise. A test walks the whole catalogue and fails if a statement contains «fracción», «decimal», «ecuación» or «incógnita». Contexts are hand-written with their own plausible ranges; the numbers are drawn each time so the answer cannot be memorised. Never add a filter by technique — it would hand back the difficulty the topic exists to train.
